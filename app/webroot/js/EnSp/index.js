...
function init(){
	...
	// CBBXS-1023
	// 綱IDリストJSON
	var bio_cls_id_json = jQuery('#bio_cls_id_json').val();
	var bioClsIdList = JSON.parse(bio_cls_id_json);
	disFilData['bio_cls_id'] ={'fil_type':'select','option':{'list':bioClsIdList}};
	// 絶滅危惧種カテゴリーIDリストJSON
	var en_ctg_id_json = jQuery('#en_ctg_id_json').val();
	var enCtgIdList = JSON.parse(en_ctg_id_json);
...
function tableTransform(mode_no){
	...
	if(mode_no == 1){
		jQuery("#en_sp_tbl").addClass('table_transform');
		jQuery("#table_transform_tbl_mode").show();
		jQuery("#table_transform_div_mode").hide();
	}else{
		jQuery("#en_sp_tbl").removeClass('table_transform');
