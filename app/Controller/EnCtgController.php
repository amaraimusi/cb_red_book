 ...
 * 
 * @note
 * 絶滅危惧種カテゴリー画面では絶滅危惧種カテゴリー一覧を検索閲覧、編集など多くのことができます。
 * 
 * @date 2015-9-16 | 2018-4-25 削除のバグを主末井
 * @version 3.0.1
 *
 */
class EnCtgController extends CrudBaseController {

	/// 名称コード
	public $name = 'EnCtg';
	
	/// 使用しているモデル
	public $uses = array('EnCtg','CrudBase');
	
	/// オリジナルヘルパーの登録
	public $helpers = array('CrudBase');

	/// デフォルトの並び替え対象フィールド
	public $defSortFeild='EnCtg.sort_no';
	 ...
	 * indexページのアクション
	 *
	 * indexページでは絶滅危惧種カテゴリー一覧を検索閲覧できます。
	 * 一覧のidから詳細画面に遷移できます。
	...
	public function index() {
		...
		// CrudBase共通処理（前）
		$crudBaseData = $this->indexBefore('EnCtg');//indexアクションの共通先処理(CrudBaseController)
		
		//一覧データを取得
		$data = $this->EnCtg->findData($crudBaseData);
...
// 		$dbExp->test($dao);
	 ...
	 * 詳細画面
	 * 
	 * 絶滅危惧種カテゴリー情報の詳細を表示します。
	 * この画面から入力画面に遷移できます。
	 * 
	 */
	public function detail() {
		
		$res=$this->edit_before('EnCtg');
		$ent=$res['ent'];
	

		$this->set(array(
				'title_for_layout'=>'絶滅危惧種カテゴリー・詳細',
				'ent'=>$ent,
		));
...
// 		}
		...
		// エンティティをDB保存
		$this->EnCtg->begin();
		$ent = $this->EnCtg->saveEntity($ent,$regParam);
		$this->EnCtg->commit();//コミット
	...
	public function ajax_delete(){
		...
		$this->EnCtg->begin();
		if($eliminate_flg == 0){
			$ent = $this->EnCtg->saveEntity($ent,$regParam); // 更新
		}else{
		    $this->EnCtg->delete($ent['id']); // 削除
		}
		$this->EnCtg->commit();//コミット
	...
	*/
	public function auto_save(){
		...
		// データ保存
		$this->EnCtg->begin();
		$this->EnCtg->saveAll($data); // まとめて保存。内部でSQLサニタイズされる。
		$this->EnCtg->commit();
	...
	public function csv_fu(){
		...
		$this->csv_fu_base($this->EnCtg,array('id','en_ctg_val','en_ctg_name','en_ctg_date','en_ctg_group','en_ctg_dt','img_fn','note','sort_no'));
	...
	private function getDataForDownload(){
        ...
        //セッションから検索条件情報を取得
        $kjs=$this->Session->read('en_ctg_kjs');
        
        // セッションからページネーション情報を取得
        $pages = $this->Session->read('en_ctg_pages');
		...
		//DBからデータ取得
	   $data=$this->EnCtg->findData($kjs,$page_no,$row_limit,$sort_field,$sort_desc);
		if(empty($data)){
			return array();
		}
	...
	private function initCrudBase(){
		...
		$this->kensakuJoken=array(
			...
			array('name'=>'kj_en_ctg_name','def'=>null),
		...
		$this->kjs_validate=array(
				...
				'kj_en_ctg_name'=> array(
						'maxLength'=>array(
								'rule' => array('maxLength', 255),
								'message' => '絶滅危惧種カテゴリー名は255文字以内で入力してください',
								'allowEmpty' => true
						),
				),
				'kj_note'=> array(
